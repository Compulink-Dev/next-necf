...
//@ts-ignore
function EditChair({ team }) {

    const router = useRouter()
    const [loading, setLoading] = useState(false)
    const [imageUrl, setImageUrl] = useState(team.image)
    const { register, handleSubmit, reset, formState: { errors } } = useForm()

    async function handleDelete() {
        try {

            const res = await fetch(`/api/chairs/${team._id}`, {
                method: 'DELETE'
            })
    ...
    async function onSubmit(data: any) {
        ...
        try {
            ...
            const response = await fetch(`/api/chairs/${team._id}`, {
                method: "PUT",
