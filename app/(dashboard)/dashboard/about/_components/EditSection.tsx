...
function EditSection({ gallery }) {
  ...
  async function handleDelete() {
    try {
      ...
      const res = await fetch(`/api/aboutSection/${gallery._id}`, {
        method: 'DELETE'
      })
  ...
  async function onSubmit(data: any) {
    ...
    try {
      ...
      const response = await fetch(`/api/aboutSection/${gallery._id}`, {
        method: "PUT",
  ...
  return (
    <div className="">
      <form onSubmit={handleSubmit(onSubmit)} action="" className="">
        <div className="mb-6">
          <Label className="text-slate-600">1. Title</Label>
          <Textarea
        ...
        <div className="mb-6">
          <Label className="text-slate-600">2. Title</Label>
          <Textarea
            defaultValue={gallery.title2}
          ...
          {errors.title && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> title2 already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6">
          <Label className="text-slate-600">3. Title</Label>
          <Textarea
            defaultValue={gallery.title3}
          ...
          {errors.title && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> title3 already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6">
          <Label className="text-slate-600">4. Title</Label>
          <Textarea
            defaultValue={gallery.title4}
          ...
          {errors.title && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> title4 already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6">
          <Label className="text-slate-600">5. Title</Label>
          <Textarea
            defaultValue={gallery.title4}
            {...register("title5", { required: true })}
            id="title5"
            className=""
            placeholder="Enter title5"
          />
          {errors.title && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> title5 already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6">
          <Label className="text-slate-600">6. Title</Label>
          <Textarea
            defaultValue={gallery.title6}
            {...register("title6", { required: true })}
            id="title6"
            className=""
            placeholder="Enter title6"
          />
          {errors.title && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> title6 already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6 flex items-center gap-4">
          ...
          />
          <div className="">
            <Label className="text-slate-600">2. Image</Label>
