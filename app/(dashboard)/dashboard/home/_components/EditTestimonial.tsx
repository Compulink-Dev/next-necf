...
//@ts-ignore
function EditTestimonial({ section }) {
  ...
  async function handleDelete() {
    try {
      ...
      const res = await fetch(`/api/testimonial/${section._id}`, {
        method: 'DELETE'
      })
  ...
  async function onSubmit(data: any) {
    ...
    try {
      ...
      const response = await fetch(`/api/testimonial/${section._id}`, {
        method: "PUT",
      ...
      <form onSubmit={handleSubmit(onSubmit)} action="" className="">
        ...
        <div className="mb-6">
          ...
          {errors.company && (
            <p className="text-red-600 text-sm">
              Oops!
              <span className=""> company already taken</span>
            </p>
          )}
        </div>
        <div className="mb-6">
